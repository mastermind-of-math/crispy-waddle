...
function game(){
  ...
  if(gameConfig.status==='start' ){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press Arrow Keys to Start Game", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatus(character){
    if((keyDown(control.right) || keyDown(control.left) || keyDown(control.up)) && gameConfig.status==="start") {
        world_start.play();
...
function enemys(enemys){
    enemys.forEach(function(enemy){
      stateOfEnemy(enemy);
        ...
        stroke(0, 100, 255)
        strokeWeight(4);
        rect(enemy.position.x - 6, enemy.position.y - 50, 36, 100);
  });
} 
...
function scores(character){
  ...
  text("Stars:: "+round(gameConfig.scores),20,40);
